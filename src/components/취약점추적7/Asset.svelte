<script>
  import { allTraceByAsset, ModalData } from "../../services/page7/trace.store";
  ...
  function handleNext() {
    if (currentModalIndex < $selectedAssetTableData[0]?.vulns.length - 1) {
      currentModalIndex++;
      const {
        ast_uuid__ass_uuid__ast_hostname: asset_name,
        ccr_item_no__ccc_item_no: chklist,
        cct_index__cct_target: target,
      } = $selectedAssetTableData[0]?.vulns[currentModalIndex];
      modalDataFunction(asset_name, chklist, target);
    } else {
          ...
          <tbody>
            ...
            >
              <td class="text-center"
                >{$selectedAssetTableData[0]?.vulns.length - index}</td
              >
  ...
  border-bottom-right-radius: 10px;">조치내역</th
        ...
        <div
          ...
          class:disabled={currentModalIndex ===
            $selectedAssetTableData[0]?.vulns.length - 1}
        ></div>
