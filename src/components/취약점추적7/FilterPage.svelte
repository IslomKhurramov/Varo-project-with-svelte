<script>
  ...
  function handleNext() {
    if (currentModalIndex < $filteredTableDataForPlan.length - 1) {
      currentModalIndex++;
      const {
        ast_uuid__ass_uuid__ast_hostname: asset_name,
        ccr_item_no__ccc_item_no: chklist,
        cct_index__cct_target: target,
      } = $filteredTableDataForPlan[currentModalIndex];
      modalDataFunction(asset_name, chklist, target);
    } else {
...
<div>
  ...
  <div class="graphCardWrap col3" style="width:100%;">
    {#if $filterData.plans && $filterData.plans.length > 0}
      {#each $filterData.plans as asset, index}
            ...
            <div class="contents">
              <div class="graph">
                <div>
                  <div
                    class="circle"
                    data-percent={asset.vulnerability_count || 0}
                    data-offset="440"
                  >
                    <svg viewBox="0 0 150 150">
                      ...
                      <circle
                        ...
                        cy="75"
                        r="70"
                        stroke={getStrokeColor(asset.vulnerability_count || 0)}
                        stroke-width="10"
                        fill="none"
                        stroke-dasharray="440"
                        stroke-dashoffset={440 -
                          (440 * (asset.vulnerability_count || 0)) / 100}
              ...
              <div class="text flex col justify-between">
                <ul>
                  ...
                  <li>
                    <span>점검일시 : </span>
                    {asset.ccp_cdate
                      ? new Date(asset.ccp_cdate).toLocaleString("ko-KR", {
                          year: "numeric",
                          month: "long",
                          day: "numeric",
                          hour: "2-digit",
                  ...
                  <li>
                    <span>관련시스템 : </span>{asset.system_count ||
                      "Unknown IP"}대
                  </li>
                </ul>
              </div>
  ...
  >
    {#if $filteredTableDataForPlan && $filteredTableDataForPlan.length > 0}
      <table class="tableList hdBorder font-size: 16px;">
        ...
        {#each $filteredTableDataForPlan as vuln, index}
          <tbody>
            <tr
              on:click={() => {
      ...
      >
        ...
        <div
          ...
          class:disabled={currentModalIndex ===
            $filteredTableDataForPlan.length - 1}
        ></div>
