<script>
  ...
  function handleNext() {
    if (currentModalIndex < $filteredTableDataForTarget.length - 1) {
      currentModalIndex++;
      const {
        ast_uuid__ass_uuid__ast_hostname: asset_name,
        ccr_item_no__ccc_item_no: chklist,
        cct_index__cct_target: target,
      } = $filteredTableDataForTarget[currentModalIndex];
      modalDataFunction(asset_name, chklist, target);
    } else {
...
<div>
  <p class="header_title">
    점검항목 : {$filteredDataTarget.ccc_item_no}과 관련된 취약점 추적
  </p>

  <div class="graphCardWrap col3" style="width:100%;">
    {#if $filteredDataTarget.plans && $filteredDataTarget.plans.length > 0}
      {#each $filteredDataTarget.plans as target, index}
          ...
          <article class="graphCard hoverCard" style="min-height: 288px;">
            <div class="contents">
              ...
              <div class="text flex col justify-between">
                <ul>
                  <li>
                    <span>프로젝트명 : </span>{target.ccp_title ||
                      "데이터 없음"}
                  </li>
                  <li>
                    <span>점검일시: </span>
  ...
  >
    {#if $filteredTableDataForTarget && $filteredTableDataForTarget.length > 0}
          ...
          <tbody>
            ...
            >
              <td class="text-center"
                >{$filteredTableDataForTarget.length - index}</td
              >
      ...
      >
        ...
        <div
          ...
          class:disabled={currentModalIndex ===
            $filteredTableDataForTarget.length - 1}
        ></div>
